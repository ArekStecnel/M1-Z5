...
def wybierz_rodzaj():                                               #funkcja zależnie od tego co wpisaliśmy wykonuje określone rzeczy
    ...
    if co_wyswietlamy == "CF":                                      #po wpisaniu CF zamienia celcjusze na farenheity
        ...
        try:
            api_address = 'http://api.openweathermap.org/data/2.5/weather?appid=d8cb46b72f35eb7b3563fba6ff9cd3fe&q='
            address = api_address + co_wyswietlamy
            json_data = requests.get(address).json()                                                                    #dane są pobierane z strony internetowej z plikiem o rozszerzeniu json
            ...
            temperatura_F =round(temperatura_F,2)
            print("W mieście",co_wyswietlamy, "temperatura wynosi",temperatura_C,"°C (",temperatura_F,"°F )")           #wypisanie danych
